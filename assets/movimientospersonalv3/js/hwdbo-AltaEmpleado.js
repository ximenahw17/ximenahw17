...
$(document).ready(function () {
    ...
    fillCombo(21, 'id_tabulador', "&_Parameters='" + _data.patron + "','" + _data.puesto + "','" + _data.periodopago + "'");
        ...
        $("#GridSubcontratante").jqxGrid(
            {
                ...
                columns: [
                    ...
                    {
                        text: 'Porcentaje', datafield: 'Porcentaje', width: '20%', align: 'center', cellsalign: 'center', editable: true, columntype: 'numberinput', cellsformat:'d3',
                        validation: function (cell, value) {
                            if (value < 0 ) {
                                return { result: false, message: "El porcentaje no puede ser negativo" };
                            }
                            else if (value > 100) {
                                return { result: false, message: "El porcentaje no puede ser mayor a 100" };
                            }
                            else {
                                return true;
                            }
...
function empleados() {
    if (obtieneParametroURL()) {       
        ...
        {
            setTimeout(function() { 
                asignaCombos(JSON.parse(atob(localStorage.getItem(IdResultadoBusqueda)))).then(r=>{
                    setTimeout(function(){
                        desabilitaControles(); 
...
function asignaValores(_data) {
    ...
    if(_data.subcontratante!==undefined && _data.subcontratante.length>0)
    {
        ...
        if(subs.length>0)
        {
            ...
            if(_data.estatus!=3)
            {
                setTimeout(function() { $('#GridSubcontratante').jqxGrid({ source: dataAdapter });},8000);
            }
        }
    }
