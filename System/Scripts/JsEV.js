...
/**
 * Genera un dataAdapter para llenar el jqxgrid. 
 * @param {object} data - Objeto array con la informacion para llenar el grid.
 */
 function GetAdapter(data) {
    ...
    if(data)
        if(data.length>0){
            ...
            keys.forEach(item=>{
                datafields.push({ name: item, type: 'string' });
            });
        }
...
$('.class-inicio').click((event) => {
    // if (window.location.href.indexOf('ConsultaEmpleado.html') >= 0) {
    //     let url = new URL(location.href);
    //     let estatus = url.searchParams.get("estatus");
    //     if (estatus)
    //         window.location.href = 'Nuevoingreso.html';
    //     else
    //         window.location.href = 'MenuMovimientosPersonal.html?siteCode=' + localStorage.getItem('hash');
    // } else
    let sitio=$(event.currentTarget).data('modulo');
    window.location.href = sitio + '.html?siteCode=' + getHSH();
});
$('.class-regresar').click(() => {
    // if (window.location.href.indexOf('ConsultaEmpleado.html') >= 0) {
    //     let url = new URL(location.href);
    //     let estatus = url.searchParams.get("estatus");
    //     if (estatus)
